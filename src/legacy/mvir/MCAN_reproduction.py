...
from FakeNet_datasets import FakeNet_dataset
...
#         self.pthfile = "/hy-tmp/lhc/Reproduction_of_MCAN-main/models/vgg19-dcbb9e9d.pth"
...
class vgg(nn.Module):
    ...
    def forward(self, img):
        ...
        # image = self.vgg(img) #[batch, num_ftrs]
        img = self.feature(img)
        image = img.view(img.size(0), img.size(1), -1).transpose(1, 2)
        # img = img.view(img.size(0), -1)
        # image = self.classifier(img)

        return image ###[B,512,49]

# class VGGPositionSummarization(nn.Module):
#     """
#     优化后的多视角图像特征提取器，结合VGG、PositionEncoder和Summarization模块
#     """
#     def __init__(self, model_dim, smry_k, pthfile, embed_size, head, drop=0.0):
#         super(VGGPositionSummarization, self).__init__()
#         self.model_dim = model_dim
#         self.pthfile = pthfile
...
#         vgg_19 = torchvision.models.vgg19(pretrained=False)
#         vgg_19.load_state_dict(torch.load(self.pthfile))
#         self.feature = vgg_19.features  # 使用VGG的卷积层特征
...
#         # 全连接层，用于将VGG特征投影到嵌入空间
#         self.fc = nn.Linear(512, embed_size)
#         self.init_weights()

#     def init_weights(self):
#         """Xavier初始化全连接层权重"""
#         r = torch.sqrt(torch.tensor(6.)) / torch.sqrt(torch.tensor(self.fc.in_features + self.fc.out_features, dtype=torch.float32))
#         self.fc.weight.data.uniform_(-r, r)
#         self.fc.bias.data.fill_(0)

#     def forward(self, img, boxes, imgs_wh):
#         # 使用VGG提取图像特征
#         img = self.feature(img)
#         img = img.view(img.size(0), img.size(1), -1).transpose(1, 2)  # [batch_size, 512, num_regions]
        
#         # 通过全连接层将VGG特征投影到嵌入空间
#         fc_img_emd = self.fc(img)
...
#         # 自适应门控自注意力机制
#         self_att_emb = self.agsa(fc_img_emd, posi_emb)
#         self_att_emb = l2norm(self_att_emb)

#         # 多视图摘要
#         smry_mat = self.mvs(self_att_emb)
#         L = F.softmax(smry_mat, dim=1)
#         img_emb_mat = torch.matmul(L.transpose(1, 2), self_att_emb)

#         return F.normalize(img_emb_mat, dim=-1), smry_mat

# class MultiViewMCAN(nn.Module):
#     def __init__(self, model_dim, smry_k, pthfile, embed_dim, num_heads):
#         super(MultiViewMCAN, self).__init__()
        
#         # 初始化 VGG 用于图像特征提取
#         self.vgg = vgg(model_dim, pthfile)
        
#         # 初始化 PositionEncoder 和 Summarization 模块
#         self.position_encoder = PositionEncoder(embed_dim=embed_dim, posi_dim=6)
#         self.summarization = Summarization(embed_size=embed_dim, smry_k=smry_k)
        
#         # 多模态注意力机制
#         self.attention = multimodal_attention(attention_dropout=0.5)
        
#     def forward(self, image, region_boxes, imgs_wh, text_features):
#         # 步骤 1：使用 VGG 提取图像特征
#         image_features = self.vgg(image)  # [batch_size, num_regions, embed_dim]
        
#         # 步骤 2：应用 PositionEncoder 编码区域位置
#         pos_encoded_features = self.position_encoder(region_boxes, imgs_wh)
        
#         # 步骤 3：应用 Summarization 压缩多视角图像特征
#         summarized_features = self.summarization(image_features)
        
#         # 将汇总后的特征与位置编码的特征结合
#         combined_features = summarized_features + pos_encoded_features
...
class Summarization(nn.Module):
    ...
    def forward(self, rgn_emb):
        ...
        # Transpose to desired output format (bs, k, num_r)
        #smry_mat = smry_mat.transpose(1, 2)  # (bs, k, num_r)
        L = F.softmax(smry_mat, dim=1)
        img_emb_mat = torch.matmul(L.transpose(1, 2),x) #(bs, k, dim)

        # print('*********************************')
        # print(img_emb_mat.size())

        return F.normalize(img_emb_mat, dim=-1)#, smry_mat

        #return smry_mat

class MCAN_Optimized(nn.Module):
    """
    MCAN optimized with Multi-View Summarization and VGG feature extraction
    """

    def __init__(self, model_dim, vgg_path, smry_k):
        super(MCAN_Optimized, self).__init__()
        self.vgg = vgg(model_dim, vgg_path)
        self.summarization = Summarization(embed_size=512, smry_k=smry_k)  # Summarization layer

    def forward(self, img):
        # Step 1: Extract visual features using VGG
        img_features = self.vgg(img)  # Output: [B, 49, 512]
        # print(img_features.size())

        # Step 2: Apply Multi-View Summarization
        summarized_features = self.summarization(img_features)  # Output: [B, k, num_r]
...
class multimodal_attention(nn.Module):
    ...
    def __init__(self, num_heads,dim_per_head,attention_dropout=0.5):
    ...
    def forward(self, q, k, v,  attn_mask=None):
        ...
        attention = torch.matmul(q, k.transpose(-2, -1)) /  (self.dim_per_head ** 0.5)
        # print('attention.shape:{}'.format(attention.shape))
        # if scale:
        #     attention = attention * scale
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1).unsqueeze(1).repeat(1, self.num_heads, 1, 1)
            attention = attention.masked_fill_(attn_mask == 0, float('-inf'))
...
class MultiHeadAttention(nn.Module):
    def __init__(self, model_dim=256, num_heads=12, dropout=0.5):
        ...
        self.linears = [self.linear_q, self.linear_k, self.linear_v]

        self.dot_product_attention = multimodal_attention(self.num_heads,self.dim_per_head,dropout)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        _, S, _ = key.size()
        query, key, value = \
            [l(x).view(B, -1, self.num_heads, self.dim_per_head).transpose(1, 2)
             for l, x in zip(self.linears, (query, key, value))]


        # scaled dot product attention

        attention = self.dot_product_attention(query, key, value , attn_mask)



        attention = attention.transpose(1, 2).contiguous().view(B, -1, D)
...
class multimodal_fusion_layer(nn.Module):
    ...
    def __init__(self, model_dim=256, num_heads=12, ffn_dim=2048, dropout=0.5):
    ...
    def forward(self, image_output, text_output, attn_mask=None):
        ...
        output = torch.cat([image_output,output_1], dim=-1)
...
class NetShareFusion(nn.Module):
    def __init__(self,
        ...
        self.fc_list = nn.ModuleList([nn.Linear(256, model_dim) for _ in range(4)])

        # text
        # self.config = BertConfig.from_pretrained(self.CASED)

        # self.bert = BertModel.from_pretrained(self.CASED, config=self.config)
        self.config = BertConfig.from_pretrained("/hy-tmp/lhc/Reproduction_of_MCAN-main-gossip/models/bert-base-uncased")
        self.bert = BertModel.from_pretrained("/hy-tmp/lhc/Reproduction_of_MCAN-main-gossip/models/bert-base-uncased", config=self.config)

        self.linear_text = nn.Linear(bert_dim, model_dim)
        self.bn_text = nn.LayerNorm(model_dim)

        self.dropout = nn.Dropout(dropout)

        # image
        self.MCAN_Optimized = MCAN_Optimized(model_dim=model_dim, vgg_path=pthfile, smry_k=4)
        self.linear_image = nn.Linear(1024, model_dim)
        self.bn_vgg = nn.LayerNorm(model_dim)
        ...
        # classifier
        self.linear1 = nn.Linear(model_dim * 2, 35)
    ...
    #             dct_img, attn_mask):
    def forward(self, text_input_ids, token_type_ids, attention_mask, image, attn_mask):

        # textual feature
        bert_output = self.bert(input_ids=text_input_ids,
                                token_type_ids=token_type_ids,
                                attention_mask=attention_mask)
        text_output = bert_output[0]  # the representation of the whole sentence
        ...
        # visual feature
        output = self.MCAN_Optimized(image)
        ...
        # 对 output 进行全局池化，确保维度与 dct_out 和 text_output 对齐
        # output = torch.mean(output, dim=1)  # 将 output 的维度减少

        # print("output shape: ", output.shape)
        # print("dct_out shape: ", dct_out.shape)
        ...
        emb_list = []
        for fc in self.fc_list: ########12
            emb = fc(output) #output
            emb = emb.permute(0, 2, 1)
            emb = max_pool1d(emb, emb.size(2)).squeeze(2)
            emb = l2norm(emb, dim=-1)
            emb_list.append(emb) #emb_list 12,2,256
        ...
        for i, emb in enumerate(emb_list):
            # 将当前视角的嵌入特征与文本和DCT特征结合
            combined = torch.cat([emb, text_output], dim=-1)  # [2, 768]
            
            # 通过线性层计算得分
            combined = F.relu(self.linear1(combined))  # [2, 35]
            combined = self.dropout(combined)
            combined = self.bn_1(combined)
            score = self.linear2(combined)  # [2, num_labels]，例如 [2, 1]，假设二分类
            
            # 选择假新闻得分最高的
            if max_score is None or score[:, 1].max() > max_score[:, 1].max():  # 比较第一个类别（假新闻）的得分
                max_score = score
                max_score_idx = i

        # 对最大得分进行softmax
        y_pred_prob = self.softmax(max_score)  # [2, num_labels]

        return max_score, y_pred_prob
        ...
        # for i, emb in enumerate(emb_list):
        #     # 将当前视角的嵌入特征与文本和DCT特征结合
        #     combined = torch.cat([emb, text_output, dct_out], dim=-1)  # [2, 768]
            
        #     # 通过线性层计算得分
        #     combined = F.relu(self.linear1(combined))  # [2, 35]
        #     combined = self.dropout(combined)
        #     combined = self.bn_1(combined)
        #     score = self.linear2(combined)  # [2, num_labels]，假设二分类
        ...
        # # 计算12个视角的平均得分
        # average_score = total_score / len(emb_list)  # len(emb_list) 应该为12

        # # 对平均得分进行softmax
        # y_pred_prob = self.softmax(average_score)  # [2, num_labels]

        # return average_score, y_pred_prob

            
        # text_output = torch.mean(text_output, dim=1)
        # #dct_out = torch.mean(dct_out, dim=1)

        # output = torch.cat([output, text_output, dct_out], dim=-1)
        ...
        # y_pred_prob = self.softmax(output) #12 ,2
...
class TrainALL(tune.Trainable):
    def collate_fn_english(self,data):
        ...
        token_uncased = BertTokenizer.from_pretrained('/hy-tmp/lhc/Reproduction_of_MCAN-main-gossip/models/bert-base-uncased')
        item = data[0]
    ...
    def get_dataloader(self):
        ...
        train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True, collate_fn=self.collate_fn_english,
        ...
        validate_loader = DataLoader(validate_dataset, batch_size=32, shuffle=False,
                                     collate_fn=self.collate_fn_english,
                                     num_workers=4, sampler=None, drop_last=False,
                                     pin_memory=True)
        self.thresh = train_dataset.thresh
    ...
    def get_optimizer(self):
        ...
        vgg_param_optimizer = list(self.model.MCAN_Optimized.named_parameters())
    ...
    def setup(self, config):
        ...
        # self.CASED = '/hy-tmp/lhc/Reproduction_of_MCAN-main/models/bert-base-chinese'  # chinese
        # self.VOCAB = '/hy-tmp/lhc/Reproduction_of_MCAN-main/models/bert-base-chinese/vocab.txt'
        
        self.CASED = '/hy-tmp/lhc/Reproduction_of_MCAN-main-gossip/models/bert-base-uncased'  # multilingual-cased
        self.VOCAB = '/hy-tmp/lhc/Reproduction_of_MCAN-main-gossip/models/bert-base-uncased/vocab.txt'

        self.pthfile = '/hy-tmp/lhc/Reproduction_of_MCAN-main-gossip/models/vgg19-dcbb9e9d.pth'
        self.save_root = '/hy-tmp/lhc/Reproduction_of_MCAN-main-gossip/results'
    ...
    def train_one_time(self):
        ...
        for epoch_index, epoch in enumerate(range(self.epochs)):
            ...
            for i, items in enumerate(self.train_loader):
                ...
                outputs = self.model(input_ids, token_type_ids , attention_mask,image,attn_mask=None)
    ...
    def print_result_table_handler(self, loss_values, acc_values,
        ...
        # 这个地方需要绝对路径，不能使用相对路径
        results_path = '/hy-tmp/lhc/Reproduction_of_MCAN-main-gossip/results'
        if not os.path.exists(results_path):
            os.makedirs(results_path)
        f_result = open(
            os.path.join(results_path, '{}_accuracy.txt'.format(self.dataset_name)), 'a+')
    ...
    def condition_save(self, epoch_index, test_epoch_acc, report):
        ...
        weibo_threshold = 0.91
        if self.dataset_name == 'twitter':
            if test_epoch_acc >= twitter_threshold:
                folder_path = '/hy-tmp/lhc/ray'
...
if __name__ == '__main__':
    ...
    analysis = tune.run(
        ...
        config={
            ...
            "dataset_name": tune.grid_search(['gossip']),
            "max_sen_len": tune.grid_search([197]),
            ...
            "epochs": tune.grid_search([100]),
            "train_bs": tune.grid_search([32]),
            "test_bs": tune.grid_search([32]),
